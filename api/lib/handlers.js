...
// Required data: phone
// Optional data: none
// Only let an authenticated user access their object. Don't let them access anyone data.
...
// Only let an authenticated user delete their object. Dont let them delete anyone else
            ...
            if(tokenIsValid) {
                ...
                _data.read('users', phone, function(err, data) {
                    if(!err && data) {
                        ...
                        callback(404, {'Error' : 'Could not delete the specified user'});
                    }
                });
            } else {
                callback(403, {'Error' : 'Missing required token in header, or token is invalid'});
...
// Handler for all the tokens methods
handlers._tokens = {};
    ...
    if(id) {
        ...
        _data.read('tokens', id, function(err, data) {
            if(!err && data) {
                ...
                callback(404, {'Error' : 'Could not delete the specified token'});
            }
        });
    } else {
        callback(400, {'Error' : 'Missing required field'});
